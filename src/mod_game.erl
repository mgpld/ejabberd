...
-record(mug_room, {name_host = {<<"">>, <<"">>} :: {binary(), binary()} |
                                                   {'_', binary()},
                   opts = [] :: list() | '_'}).

-record(mug_online_room,
        {name_host = {<<"">>, <<"">>} :: {binary(), binary()} | '$1' |
                                         {'_', binary()} | '_',
         pid = self() :: pid() | '$2' | '_' | '$1'}).

-record(mug_registered,
...
% h(armony)_multi_user_game
-define(PROCNAME, h_mug).
...
shutdown_rooms(Host) ->
    MyHost = gen_mod:get_module_opt_host(Host, mod_muc,
					 <<"conference.@HOST@">>),
    Rooms = mnesia:dirty_select(mug_online_room,
				[{#mug_online_room{name_host = '$1',
						   pid = '$2'},
				  [{'==', {element, 2, '$1'}, MyHost}],
				  ['$2']}]),
...
create_room(Host, GameId, From, Nick, Opts) ->
    Proc = gen_mod:get_module_proc(Host, ?PROCNAME),
    gen_server:call(Proc, {create, GameId, From, Nick, Opts}).

% create_room(<<"messaging.harmony">>, 1, undefined, <<"abcd">>, []).

route(Host, GameId, From, Function, Args) ->
    Proc = gen_mod:get_module_proc(Host, ?PROCNAME),
    Proc ! {route, GameId, From, Function, Args}.

% Called by the game_master process
...
store_room(_LServer, Host, Name, Opts, mnesia) ->
    F = fun () ->
		mnesia:write(#mug_room{name_host = {Name, Host},
				       opts = Opts})
	end,
...
restore_room(_LServer, Host, Name, mnesia) ->
    case catch mnesia:dirty_read(mug_room, {Name, Host}) of
      [#mug_room{opts = Opts}] -> Opts;
      _ -> error
    end;
...
forget_room(_LServer, Host, Name, mnesia) ->
    F = fun () -> mnesia:delete({mug_room, {Name, Host}})
	end,
    mnesia:transaction(F);
...
can_use_nick(_LServer, Host, JID, Nick, mnesia) ->
    ...
    LUS = {LUser, LServer},
    case catch mnesia:dirty_select(mug_registered,
				   [{#mug_registered{us_host = '$1',
						     nick = Nick, _ = '_'},
				     [{'==', {element, 2, '$1'}, Host}],
				     ['$_']}])
	of
      {'EXIT', _Reason} -> true;
      [] -> true;
      [#mug_registered{us_host = {U, _Host}}] -> U == LUS
    end;
...
init([Host, Opts]) ->
    ...
    case gen_mod:db_type(Opts) of
        mnesia ->
            mnesia:create_table(mug_room,
                                [{disc_copies, [node()]},
                                 {attributes,
                                  record_info(fields, mug_room)}]),
            mnesia:create_table(mug_registered,
                                [{disc_copies, [node()]},
                                 {attributes,
                                  record_info(fields, mug_registered)}]),
            update_tables(MyHost);
            %mnesia:add_table_index(mug_registered, nick);
        _ ->
            ok
    end,
    mnesia:create_table(mug_online_room,
			[{ram_copies, [node()]},
			 {attributes, record_info(fields, mug_online_room)}]),
    mnesia:add_table_copy(mug_online_room, node(), ram_copies),
...
handle_call({create, GameId, From, GameRef, Opts}, _From,
    ...
    ?DEBUG("MUG: create new room '~s' (~p) mod: ~p", [GameRef, GameId, Module]),
    NewOpts = case Opts of
		default -> DefOpts;
		_ -> Opts
	      end,
    Ident = fun(A) -> A end,
    %GameId = get_mod:get_opt(id, Opts, Ident, undefined),
    {ok, Pid} = game_master:start_link(GameId, Module, GameRef),
    register_room(Host, GameRef, Pid),
    {reply, ok, State}.
...
handle_info({route, GameId, From, Function, Args},
    ...
    ?DEBUG("route: ~p ~p ~p ~p", [ GameId, From, Function, Args ]),
...
% Game as stopped, do cleanup    
handle_info({room_destroyed, RoomHost, Pid}, State) ->
    F = fun () ->
		mnesia:delete_object(#mug_online_room{name_host =
							  RoomHost,
						      pid = Pid})
	end,
    mnesia:transaction(F),
...
handle_info({'EXIT', Pid, Reason}, State) ->
    %?INFO_MSG(?MODULE_STRING " Unregistering game pid: ~p: reason: ~p", [ Pid, Reason ]),
    F = fun() ->
	Es = mnesia:select( mug_online_room,
	       [{#mug_online_room{pid = '$1', _ = '_'},
		 [{'==', '$1', Pid}],
		 ['$_']}]),
	lists:foreach(fun(E) ->
	    #mug_online_room{name_host = GameRef } = E,
	    ?INFO_MSG(?MODULE_STRING " Unregistering game pid: ~p: Ref: ~p reason: ~p", [ Pid, GameRef, Reason ]),
...
do_route1(Host, ServerHost, Access, HistorySize, RoomShaper,
	...
	_ ->
	    case mnesia:dirty_read(mug_online_room, {Room, Host}) of
		[] ->
	    ...
	    [R] ->
		Pid = R#mug_online_room.pid,
		?DEBUG("Game: send to process ~p~n", [Pid]),
		%mod_muc_room:route(Pid, From, Nick, Packet),
		ok
	  end
    end.

% do_route(Host, ServerHost, From, GameRef, next, Args) ->
%     case mnesia:dirty_read(mug_online_room, {GameRef, Host}) of
% 	[] ->
% 	    ?ERROR_MSG("Room ~p (~p) is not available", [ GameRef, Host ]),
% 	    ok;
% 
% 	[R] ->
% 	    Pid = R#mug_online_room.pid,
% 	    ?DEBUG("Game: send to process ~p ~p ~p\n", [Pid, Function, Args]),
% 	    case apply(game_master, Function, [Pid|Args]) of 
% 		ok ->
% 		    ok;
% 
% 	    %mod_muc_room:route(Pid, From, Nick, Packet),
% 	    ok
%     end;
do_route(Host, ServerHost, From, GameRef, Function, Args) ->
    case mnesia:dirty_read(mug_online_room, {GameRef, Host}) of
	[] ->
	    ?ERROR_MSG("Room ~p (~p) is not available", [ GameRef, Host ]),
	    ok;
	...
	[R] ->
	    Pid = R#mug_online_room.pid,
	    ?DEBUG(?MODULE_STRING " Game: send to process ~p ~p ~p\n", [Pid, Function, Args]),
	    apply(game_master, Function, [Pid|Args]),
	    %mod_muc_room:route(Pid, From, Nick, Packet),
	    ok
    end.
...
get_rooms(_LServer, Host, mnesia) ->
    case catch mnesia:dirty_select(mug_room,
				   [{#mug_room{name_host = {'_', Host},
					       _ = '_'},
				     [], ['$_']}])
	of
...
load_permanent_rooms(Host, ServerHost, Access, HistorySize, RoomShaper) ->
    lists:foreach(
      fun(R) ->
              {Room, Host} = R#mug_room.name_host,
              case mnesia:dirty_read(mug_online_room, {Room, Host}) of
                  [] ->
...
register_room(Host, Room, Pid) ->
    ?DEBUG(?MODULE_STRING " Registering room: ~p, pid: ~p", [ {Room, Host}, Pid ]),
    F = fun() ->
	mnesia:write(#mug_online_room{
	    name_host = {Room, Host},
	    pid = Pid})
	end,
    mnesia:transaction(F).
...
iq_disco_items(Host, From, Lang, none) ->
    lists:zf(fun (#mug_online_room{name_host =
				       {Name, _Host},
				   pid = Pid}) ->
...
get_vh_rooms(Host, #rsm_in{max=M, direction=Direction, id=I, index=Index})->
    ...
    L = lists:sort(
	  mnesia:dirty_select(mug_online_room,
			      [{#mug_online_room{name_host = '$1', _ = '_'},
				Guard,
				['$_']}])),
    L2 = if
    ...
    if L2 == [] -> {L2, #rsm_out{count = Count}};
	   ...
	   T = lists:last(L2),
	   {F, _} = H#mug_online_room.name_host,
	   {Last, _} = T#mug_online_room.name_host,
	   {L2,
...
get_room_pos(Desired, [HeadRoom | _], HeadPosition)
    when Desired#mug_online_room.name_host ==
	   HeadRoom#mug_online_room.name_host ->
    HeadPosition;
...
get_nick(_LServer, Host, From, mnesia) ->
    ...
    LUS = {LUser, LServer},
    case catch mnesia:dirty_read(mug_registered,
				 {LUS, Host})
	of
      {'EXIT', _Reason} -> error;
      [] -> error;
      [#mug_registered{nick = Nick}] -> Nick
    end;
...
set_nick(_LServer, Host, From, Nick, mnesia) ->
    ...
    F = fun () ->
		case Nick of
		  <<"">> ->
		      mnesia:delete({mug_registered, {LUS, Host}}), ok;
		  _ ->
		      Allow = case mnesia:select(mug_registered,
						 [{#mug_registered{us_host =
								       '$1',
								   nick = Nick,
								   _ = '_'},
						   [{'==', {element, 2, '$1'},
				  ...
				  of
				[] -> true;
				[#mug_registered{us_host = {U, _Host}}] ->
				    U == LUS
			      end,
		      if Allow ->
			     mnesia:write(#mug_registered{us_host = {LUS, Host},
							  nick = Nick}),
			     ok;
			 true -> false
		      end
...
update_muc_room_table(_Host) ->
    Fields = record_info(fields, mug_room),
    case mnesia:table_info(mug_room, attributes) of
      Fields ->
          ejabberd_config:convert_table_to_binary(
            mug_room, Fields, set,
            fun(#mug_room{name_host = {N, _}}) -> N end,
            fun(#mug_room{name_host = {N, H},
                          opts = Opts} = R) ->
                    R#mug_room{name_host = {iolist_to_binary(N),
                                            iolist_to_binary(H)},
                               opts = opts_to_binary(Opts)}
            end);
      _ ->
	  ?INFO_MSG("Recreating mug_room table", []),
	  mnesia:transform_table(mug_room, ignore, Fields)
    end.

update_muc_registered_table(_Host) ->
    Fields = record_info(fields, mug_registered),
    case mnesia:table_info(mug_registered, attributes) of
      Fields ->
          ejabberd_config:convert_table_to_binary(
            mug_registered, Fields, set,
            fun(#mug_registered{us_host = {_, H}}) -> H end,
            fun(#mug_registered{us_host = {{U, S}, H},
                                nick = Nick} = R) ->
                    R#mug_registered{us_host = {{iolist_to_binary(U),
                                                 iolist_to_binary(S)},
                                                iolist_to_binary(H)},
	  ...
	  mnesia:transform_table(mug_registered, ignore, Fields)
    end.

export(_Server) ->
    [{mug_room,
      fun(Host, #mug_room{name_host = {Name, RoomHost}, opts = Opts}) ->
     ...
     {mug_registered,
      fun(Host, #mug_registered{us_host = {{U, S}, RoomHost},
                                nick = Nick}) ->
              case str:suffix(Host, RoomHost) of
                  true ->
...
import(_LServer) ->
    [{<<"select name, host, opts from muc_room;">>,
      fun([Name, RoomHost, SOpts]) ->
              ...
              #mug_room{name_host = {Name, RoomHost},
                        opts = Opts}
      end},
     {<<"select jid, host, nick from muc_registered;">>,
      fun([J, RoomHost, Nick]) ->
              ...
              #mug_registered{us_host = {{U, S}, RoomHost},
                              nick = Nick}
      end}].

import(_LServer, mnesia, #mug_room{} = R) ->
    mnesia:dirty_write(R);
import(_LServer, mnesia, #mug_registered{} = R) ->
    mnesia:dirty_write(R);
import(_, _, _) ->
    pass.
...
test() ->
    ...
    %[{mug_online_room,{<<"abcd">>,
    %                   <<"conference.messaging.harmony">>}, GamePid}] = mnesia:dirty_read(mug_online_room, {<<"abcd">>,<<"conference.messaging.harmony">>}),
    User1 = <<"user1@h">>,
    User2 = <<"user3@h2">>,
    % Add two players
    Me = self(),
